...
`/sesh` does not start a second pi process. The picker returns the selected session and options to the extension, which calls pi's official `ctx.switchSession()` API. If the current agent is working, Sesh waits for it to finish before switching; it does not abort the run. Closing the picker leaves the current session running. This is a deferred switch, not two concurrent sessions. Reloading or replacing the current session cancels a queued switch. Standalone `pisesh` keeps its shell behavior and starts `pi --session`. Custom cwd overrides require a pi version that supports `cwdOverride` on extension session switches; pisesh warns if pi ignores one.
...
| ---------------------------- | ------------------------------------------------------------ |
| `↑` `↓` / `j` `k`            | move cursor                                                  |
| `Tab` / `h` / `l`            | switch tab (`Conversations` ↔ `Tasks`)         |
...
`Tab` switches between **Conversations** and **Tasks**. Within Conversations,
`[` / `]` switches between **★ Favorites**, **Today**, **Here**, and **All**.
These views filter sessions; they do not merge conversation histories.

Tasks uses the `tasks` CLI on PATH with schema version 1 (`tasks api lists`,
`snapshot`, and `exec`). Sesh does not run migration or sync commands.

- `l` opens the searchable list picker; `[` / `]` cycles task lists.
- `Space` toggles the selected task between completed and pending.
- `n` adds a task to the selected list (the current tasks-go list in All lists).
- `e` edits the selected task. Add/edit shows title, due date, and description.
  `Tab` / `Shift-Tab` / `↑↓` changes field. `Enter` moves to the next field;
  on **Save changes** it saves. `Ctrl-S` saves from any field; `Esc` cancels.
  `←` / `→`, `Home`, `End`, `Delete`, and `Backspace` edit at the cursor.
  `Ctrl-U` clears the field. Long fields scroll to keep the cursor visible.
  A blank due date clears it. The destination list is shown before saving.
- `s` cycles status: **All** (default), **Pending**, **Completed**.
- `/` enters search; `Enter` applies it. Search, list, and status filters work together.
- `↑` / `↓` selects a task. On the board, `←` / `→` selects a nonempty column;
  `Home` / `End` selects its first/last card and `PageUp` / `PageDown` scrolls by a page.
  `Enter` or `d` opens details; `↑↓` scrolls details. `Tab` still switches tabs.
- `r` reloads task lists and tasks. `Esc` clears search, then quits; in details it goes back.

Wide terminals show cards in **Overdue**, **Today**, **Upcoming**, **No date**, and
**Completed** columns, using the local calendar date. Empty columns hide except Today.
Columns scroll horizontally with selection. Narrow or short terminals use the list.
Tasks sort by due date within columns; the list sorts by due date regardless of completion.
Cards show completion, title, due date, and subtask progress; All lists also shows list names.
Search matches title, description,
ID, list, and subtask titles. Status filters use the task's completion state, not its subtasks.
All lists shows list
placements: a task referenced in Today can also appear in its source list.
Session actions such as star, rename, title generation, and resume do not apply to tasks.
Saves use stable task IDs and the loaded revision. Conflicts do not overwrite newer
changes: cancel the form, reload with `r`, and edit again. Saves reload all list
placements. Missing CLI and save/read errors appear in Tasks; Conversations remains available.
